...
class ReviewTab {
    ...
    public function review_tab_shortcode() {
        ...
        $tabs .= do_shortcode('[us_grid post_type="ids" ids="' . esc_attr($imploded_ids) . '" columns="1" items_layout="4602" overriding_link="%7B%22url%22%3A%22%22%7D"]');
        $tabs .= ' [/vc_tta_section]';

        $tabs .= '[vc_tta_section title="Reviews for this Stylist" el_class="reviews_of_beautician"] ';
        $tabs .= do_shortcode('[us_grid post_type="ids" ids="' . esc_attr($imploded_ids_b) . '" columns="2" items_layout="771" overriding_link="%7B%22url%22%3A%22%22%7D"]');
        $tabs .= '[/vc_tta_section]';
