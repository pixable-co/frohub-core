...
class GetUpcomingBookings {
    ...
    public function get_upcoming_bookings_shortcode() {
        ...
        $now = current_time('Y-m-d H:i'); // Get current site time properly
        ...
        if (empty($orders)) {
            ...
            foreach ($orders as $order) {
                ...
                if (!$appointment_datetime) {
                    continue; // No appointment set, skip
                }

                // Compare if appointment is in the future
                if ($appointment_datetime < strtotime($now)) {
                    continue; // Skip past appointments
                }

                // If we reach here, it's a future booking
                $found_upcoming = true;
