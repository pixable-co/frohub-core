...
class CancelOrderButton
{
    ...
    public function cancel_order_button_shortcode()
    {
        ...
        $this->render_cancel_reason_modal($order_id);

        // Only if order is on-hold, processing, or rescheduling
        if ($status === 'on-hold' || $status === 'processing' || $status === 'rescheduling') {
            ...
            // Show button only for on-hold, processing
            if ($status === 'on-hold' || $status === 'processing' ){
            ?>
            <button class="modal-trigger w-btn us-btn-style_6 w-btn-underlined" data-modal="<?php echo esc_attr($modal_id); ?>"
            ...
            <script type="text/javascript">
                jQuery(document).ready(function ($) {
                    ...
                    function handleReasonSubmit(button, orderId, action) {
                        ...
                        $.post("<?php echo admin_url('admin-ajax.php'); ?>", {
                            ...
                            if (response.success) {
                                ...
                                } else if (action === "decline_new_proposed_time") {
                                    // 🔹 Custom decline message
                                    title = "Proposed Appointment Declined";
                                    message = "Your booking request has been cancelled. No payment has been charged.";
                                }
                    ...
                    $(".confirm-normal-cancel-order").click(function () {
                        ...
                        modal.find(".submit-final-cancel-early, .submit-final-cancel-late, .submit-final-decline-rescheduled").hide();
                        modal.show();
                    });
                    ...
                    $(".confirm-early-cancel-order").click(function () {
                        ...
                        modal.find(".submit-final-cancel-normal, .submit-final-cancel-late, .submit-final-decline-rescheduled").hide();
                        modal.show();
                    });
                    ...
                    $(".confirm-late-cancel-order").click(function () {
                        ...
                        modal.find(".submit-final-cancel-normal, .submit-final-cancel-early, .submit-final-decline-rescheduled").hide();
                        modal.show();
                    });

                    // Declining Reschedule Time (fix: inside same scope as handleReasonSubmit)
                    $(document).on("click", ".submit-final-decline-rescheduled", function () {
                        handleReasonSubmit(this, $(this).data("order-id"), "decline_new_proposed_time");
                    });

                    // Cancelling Pending Booking
                    $(".submit-final-cancel-normal").click(function () {
                        handleReasonSubmit(this, $(this).data("order-id"), "cancel_order");
                    });

                    // Cancelling Early Booking
                    $(".submit-final-cancel-early").click(function () {
                        handleReasonSubmit(this, $(this).data("order-id"), "early_cancel_order");
                    });

                    // Cancelling Late Booking
    ...
    private function render_modals($order_id)
    {
        ...
        <div id="normalCancelModal_<?php echo esc_attr($order_id); ?>" class="status-modal">
            <div class="modal-content">
                <div class="modal-header">
                    <h5>Cancel Booking?</h5><span class="close-modal">×</span>
                </div>
        ...
        <div id="earlyCancelModal_<?php echo esc_attr($order_id); ?>" class="status-modal">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>You're within the early cancellation window. Your deposit will be refunded but the booking fee is
                        non-refundable.</p>
                </div>
                <div class="modal-footer">
        ...
        <div id="lateCancelModal_<?php echo esc_attr($order_id); ?>" class="status-modal">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p>Since your appointment is within 7 days, this counts as a late cancellation. Your deposit and booking fee
        ...
        <div id="cancelSuccessModal" class="status-modal" style="display:none;">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 id="cancel-success-title">Booking Cancelled</h5>
                </div>
                <div class="modal-body">
                    <p id="cancel-success-message">Your booking has been successfully cancelled.</p>
                </div>
                <div class="modal-footer">
    ...
    private function render_cancel_reason_modal($order_id)
    {
        ?>
        ...
        <div id="cancelReasonModal_<?php echo esc_attr($order_id); ?>" class="status-modal cancel-reason-modal">
            <div class="modal-content">
                <div class="modal-header">
                    <h5> Help us improve — why are you cancelling?</h5><span class="close-modal">×</span>
                </div>
                <div class="modal-body">
                    <p>We’d love to know why you cancelled. Your feedback helps us improve the booking experience.</p>
                    <p id="cancel-reason-error-<?php echo esc_attr($order_id); ?>" class="cancel-error-msg"
                        style="color:#c00; display:none;"></p>
                    <form id="cancel-reason-form-<?php echo esc_attr($order_id); ?>">
                        <label><input type="radio" name="reason" value="I had a scheduling conflict"> I had a scheduling
                            conflict</label><br>
                        <label><input type="radio" name="reason" value="I changed my mind"> I changed my mind</label><br>
                        <label><input type="radio" name="reason" value="The stylist didn’t respond"> The stylist didn’t
                            respond</label><br>
                        <label><input type="radio" name="reason" value="The stylist asked me to cancel"> The stylist asked me to
                            cancel</label><br>
                        <label><input type="radio" name="reason" value="The rescheduled time didn’t work for me"> The
                            rescheduled time didn’t work for me</label><br>
                ...
                <div class="modal-footer">
                    <button class="w-btn us-btn-style_6 w-btn-underlined submit-final-decline-rescheduled"
                        data-order-id="<?php echo esc_attr($order_id); ?>" style="display:none;">
                        <span class="spinner hidden"></span><span class="btn-text">Decline New Time</span>
                    </button>
                    <button class="w-btn us-btn-style_6 w-btn-underlined submit-final-cancel-normal"
                        data-order-id="<?php echo esc_attr($order_id); ?>" style="display:none;">
                        <span class="spinner hidden"></span><span class="btn-text">Cancel Booking</span>
                    </button>
                    <button class="w-btn us-btn-style_6 w-btn-underlined submit-final-cancel-early"
                        data-order-id="<?php echo esc_attr($order_id); ?>" style="display:none;">
                        <span class="spinner hidden"></span><span class="btn-text">Cancel Booking</span>
                    </button>
                    <button class="w-btn us-btn-style_6 w-btn-underlined submit-final-cancel-late"
                        data-order-id="<?php echo esc_attr($order_id); ?>" style="display:none;">
                        <span class="spinner hidden"></span><span class="btn-text">Cancel Booking</span>
                    </button>
                </div>
