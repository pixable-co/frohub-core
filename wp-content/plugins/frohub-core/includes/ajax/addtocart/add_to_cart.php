...
public function add_to_cart() {
    ...
    public function add_order_item_meta($item_id, $values) {
        ...
        // Save Total Due with 2 decimal places
        if (isset($values['deposit_due'])) {
            wc_add_order_item_meta($item_id, 'Total due on day', number_format((float)$values['deposit_due'], 2));
        }
        ...
        if (isset($values['booking_date']) && isset($values['booking_time'])) {
            ...
            // Convert start time & date to DateTime
            $start_datetime = DateTime::createFromFormat('H:i Y-m-d', trim($start_time) . ' ' . $selected_date);
    
            if ($start_datetime) {
                // Get Duration (Assuming duration is in minutes from selected add-ons)
                $duration_minutes = 0;
                if (isset($values['selected_add_ons'])) {
                    foreach ($values['selected_add_ons'] as $add_on) {
                        if (isset($add_on['duration_minutes'])) {
