    ...
    public function user_conversations() {
        ...
        if ($query->have_posts()) {
            while ($query->have_posts()) {
                ...
                if (!empty($partner) && is_object($partner) && isset($partner->ID)) {
                    ...
                    if ($thumbnail_id) {
                        $partner_image_url = wp_get_attachment_url($thumbnail_id);
                    } else {
                        // Fallback to ACF field
                        $acf_image = get_field('profile_image', $partner_id);
                        $partner_image_url = is_array($acf_image) ? ($acf_image['url'] ?? null) : $acf_image;
                    }
                }
                ...
                if ($customer && is_a($customer, 'WP_User')) {
                    ...
                    $customer_image_url = $avatar_attachment_id
                        ? wp_get_attachment_url($avatar_attachment_id)
                        : get_avatar_url($customer_id, ['size' => 96]);
                }
